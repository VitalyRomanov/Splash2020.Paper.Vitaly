...
\title{Assessing the importance of global relationships for source code modeling using GNNs}
...
\keywords{GAT, RGCN, GNN, source code, graph, neural networks}

\abstract{
Many current approaches for source code analysis rely on the token level representation of the source code. However, this representation struggles to capture long-distance dependencies in the source code, and especially global inter-project level dependencies. However, it is not clear whether it is a problem. The importance of the global level relationships between entities in the source code for solving machine learning tasks is not yet identified. In this study, we try to shed some light on this issue and analyze to which extent these global relationships can be used to solve a set of simple machine learning tasks on source code. We represent source code in the form of the global relationship graph and formulate several problems for link prediction such as name prediction, call prediction, type prediction, and other. Our findings show that information implicitly stored in global relationships can be used to solve these tasks to some extent. The information about the definitions of functions and methods seem to affect the performance of our models the most. We find that embeddings for nodes in the global graph trained with GNNs can be used for solving other tasks as well. We demonstrate that these embeddings achieve the best performance on transfer learning tasks when trained on multiple objectives simultaneously.
}
